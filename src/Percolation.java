...
public class Percolation 
{
	...
	private int xyTo1D(int x, int y)  //translate 2D points to 1D numbers
	{
		int num = (x - 1) * N + y;
		return num;
	}
	...
	public boolean isOpen(int i, int j)  //check if the site is opened
	{
		return opened[xyTo1D(i, j)] == 1;
	}
	
	public boolean isFull(int i, int j)  //check if the first row and (i, j) are connected
	{
		return UF.connected(0, xyTo1D(i, j));
	}
	
	public boolean percolates()  //check if the system is percolated(if 0 and N * N + 1 are connected)
	{
